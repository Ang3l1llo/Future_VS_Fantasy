	...
	if velocity.length() > 0:
		...
		if velocity.x != 0:
			...
			if facing_right:
				...
				attack_shape1.position = Vector2(0, attack_shape1.position.y)
				attack_shape2.position = Vector2(36, attack_shape2.position.y)
				
			else:
				# Invertir escala y ajustar posición para ataques a la izquierda
				attack_shape1.scale.x = -1
				attack_shape2.scale.x = -1
				attack_shape1.position = Vector2(68, attack_shape1.position.y)  
				attack_shape2.position = Vector2(32, attack_shape2.position.y)
